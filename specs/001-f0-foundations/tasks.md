...
- [ ] T008 [P] Add a generic, schema-introspecting build-time check (a script over Drizzle's schema
...
- [ ] T009 [P] Scaffold `test/guarded.manifest.json` with an entry for each of the fifteen G-D
...
- [ ] T010 Create the Drizzle schema file for `Application` in `src/modules/casting/schema.ts`
...
- [ ] T011 [P] [US1] Guarded test **G-D10/AC-0.7** (pool-reuse leak) in
      ...
      T009.
- [ ] T012 [P] [US1] Test in `tests/unit/lint/session-context.test.ts`: a bare `SET` (no `LOCAL`)
...
- [ ] T013 [P] [US1] Guarded test (household-scoping half of AC-0.6, via the policy layer) in
...
- [ ] T014 [P] [US1] Guarded test (household-scoping half of AC-0.6, via raw SQL) in
...
- [ ] T015 [US1] Add the RLS policy for `Application` in `src/modules/casting/schema.ts` using
...
- [ ] T016 [US1] Confirm the application's Postgres role is not the owner of the `Application`
      ...
      not just documented.
- [ ] T017 [US1] Create the central repository/policy object in
...
- [ ] T019 [P] [US2] Test in `tests/unit/casting/state-machine.test.ts`: after migration, all
      eleven states declared in `docs/03-PRD.md` §4.2.1 exist as valid enum values (FR-0.9).
- [ ] T020 [P] [US2] Test (same file): every `(from, to)` pair *not* present as a row in
      `docs/03-PRD.md` §4.2.1 throws when attempted — no silent fallthrough, no "unknown state"
      branch (FR-0.10, EC-0.7).
- [ ] T021 [P] [US2] Test in `tests/unit/audit/backward-transition.test.ts`: a permitted backward
...
- [ ] T022 [US2] Create the `ActivityEvent` schema in `src/modules/audit/schema.ts`, fields per
...
- [ ] T023 [US2] Implement the transition function in `src/modules/casting/repository.ts` (extends
...
- [ ] T024 [US2] Ensure `state` is the only field read to determine `Application` behavior
...
- [ ] T026 [P] [US3] Test in `tests/unit/audit/immutability.test.ts`: an `UPDATE` or `DELETE` on an
...
- [ ] T027 [P] [US3] Test in `tests/unit/audit/payload-allowlist.test.ts`: a write with a payload
...
- [ ] T028 [P] [US3] Test (same file): simulated end-of-retention redaction sets only the 🔴/⚫
...
- [ ] T029 [P] [US3] Guarded test (household-scoping for `ActivityEvent`, via the policy layer,
...
- [ ] T030 [P] [US3] Guarded test (household-scoping for `ActivityEvent`, via raw SQL) in
...
- [ ] T031 [US3] Add a database-level guard against `UPDATE`/`DELETE` on `ActivityEvent` (a
...
- [ ] T032 [US3] Add the RLS policy for `ActivityEvent` in `src/modules/audit/schema.ts` using
...
- [ ] T033 [US3] Confirm the application's Postgres role is not the owner of the `ActivityEvent`
      table (G-C2) — mirrors T016 for the second `household_id`-carrying table.
- [ ] T034 [US3] Implement the per-`event_type` payload positive-list validator in
...
- [ ] T035 [US3] Implement the retention-redaction path that sets 🔴/⚫ payload fields to `null`
...
- [ ] T037 Run T008's RLS-coverage build check against the complete schema (`Application` and
...
- [ ] T038 Run `bash tools/check-refs.sh --quiet` and confirm it still exits 0 — this feature's own
...
- [ ] T040 Confirm no `.skip`/`.only`/commented-out body exists in any file containing a
