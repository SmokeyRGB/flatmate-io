...
> **Status:** Entwurf V0.2 · 2026-08-19
...
> **S-38 bis S-41** in `02-SRD.md` §5.3 (Stand dort: V0.4).
...
| E-22 | **Benachrichtigungen in v1: In-App + E-Mail, Digest als Standard.** Ereignisauswahl in `HouseholdSettings` **und** persönlichen Einstellungen. Web Push in v1.1 mit dokumentiertem iOS-Vorbehalt (nur für zur Startseite hinzugefügte PWAs). `Notification`-Inhalte unterliegen derselben Sichtbarkeitspolicy wie die Anwendung. | Einzelbenachrichtigungen bei Dutzenden Bewerbungen erzeugen Abschaltung statt Beteiligung. Der Sichtbarkeitsvorbehalt ist notwendig, weil eine Benachrichtigung der einfachste Weg wäre, E-12 zu umgehen. |
| E-23 | **Geschäftsmodell: Non-Profit / spendenfinanziert. Für Bewohnende dauerhaft kostenlos.** Freemium erst in späteren Versionen für Vermietende mit mehreren Wohnungen. | Studentischer Kontext, und das Casten ist stressig genug. Die zahlende Stufe ist genau die, auf der AGG und AI Act greifen. |
...
1. Haushalts- und Bewohner-Onboarding (`Household`, `Account`, `ResidentProfile`, `Membership`, ein Beitrittscode)
2. `CastingRound` mit `Room`-Zuordnung und eigener Zustandsmaschine
3. Bewerbererfassung: Formular + regelbasierter Paste-Parser (E-15)
...
8. `CastingNote`s mit strukturierten Prompts
9. Abstimmung Runde 2 mit `Veto` (E-11)
10. Kalenderansicht über `Appointment`s und Einzugstermine
11. `ActivityEvent`-Feed und `Notification`s (In-App + E-Mail, Digest-Standard)
12. Installierbare PWA (P-2, ADR-011)
