...
permissions or role recorded on it. Revocation itself comes from FR-1.26's *remove member*
(`Membership.revoked_at`, `domain/identity.md` §2.1). That a revoked membership then grants nothing
is implemented behaviour with no requirement of its own — recorded here because it is the guarantee
every other requirement in this capability rests on, not because a packet states it.
