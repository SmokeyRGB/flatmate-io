...
BEGIN
  ...
  UPDATE public.profiles SET status = 'moved_out', is_voter = false
  ...
  VALUES
   (rnd, hh, 'Lisa Petersen', 27, 'lisa.petersen@posteo.de',
    'Hallo zusammen! Ich bin Lisa, 27, und arbeite seit zwei Jahren als Hebamme im Kreißsaal — Schichtdienst, aber ich bin ein sehr leiser Mensch, wenn ich nachts heimkomme. Ich suche eine neue WG, weil meine alte sich auflöst: zwei von uns ziehen mit ihren Partner:innen zusammen, und allein wohnen ist nichts für mich. Ich koche gern und viel zu viel, deshalb isst bei mir eigentlich immer jemand mit. Sonntags backe ich Brot, das kann man auch überleben, wenn man selbst nicht mitbackt. Ich hätte Lust auf eine WG, in der man zusammen isst, wenn es passt, und sich nicht schlecht fühlt, wenn es mal nicht passt.',
...
REVOKE ALL ON FUNCTION public.seed_demo_household() FROM PUBLIC, anon, authenticated;
...
DECLARE
  me uuid := public.current_profile_id();
BEGIN
  IF me IS NULL OR public.current_household_id() <> '11111111-1111-4111-8111-111111111111'::uuid THEN
  ...
  END IF;
  PERFORM public.seed_demo_household();
END;
$$;
