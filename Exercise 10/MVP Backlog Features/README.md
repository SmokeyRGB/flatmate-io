...
| **R5** | **Solo capacity against the scope.** `review-log.md:53` logs an open 🔴: *"Keine Aufwandsschätzung, nirgends"* | The v0.1/v0.2 split in §5.4 **is** the counter-check that gap demands — but it is a cut, not an estimate | Five features, one form path, no solver, no notifications, no PWA |
