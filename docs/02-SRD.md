...
| **O-09** | **S-50 sagt, `CastingRound` setze ein `ResidentProfile` voraus — drei bereits beschlossene Verwaltungsrechte widersprechen dem.** (1) Bildschirm **O17 Aufbewahrung** verwaltet „Aufbewahrungsfristen abgeschlossener Runden" und ist ausdrücklich ohne `ResidentProfile` erreichbar. (2) **S-35** verbietet Änderungen am Abstimmungsverfahren „nicht während laufender Runde" — das setzt voraus, dass die Verwaltung eine laufende Runde erkennen kann. (3) `Room` → `not_available` heißt „fällt aus der **laufenden Runde**". Auch `invarianten.md` §5.2 lässt den Haushalts-Account Runden sehen | Ohne Auflösung steht eine bestätigte Scope-Zeile gegen drei Bildschirme und ein Prädikat. Wer S-50 wörtlich umsetzt, baut O17 unbaubar | **Vorschlag: S-50 präzisieren statt V-2 ändern.** Sichtbar wird **Rundenidentität und Lebenszyklus** — Existenz, `title`, `status`, `room_ids`, `closed_at`, `retention_*`. Unsichtbar bleibt **alles aus `Application` Abgeleitete, ausdrücklich einschließlich Zählern** (Bewerbungszahl, „5 von 7 haben abgestimmt"): `redaction_subjects()` ist für einen Haushalts-Account **leer** (ADR-013), V-1 greift dort also nicht, und Aggregate sind laut `invarianten.md` §5.1 V-1-geschützt. Ohne diese Grenze öffnet die Präzisierung ein Leck, das es heute nicht gibt. Zu prüfen ist außerdem `CastingRound.title` — das einzige Freitextfeld in der sichtbaren Hälfte. Braucht einen eigenen Record, weil es eine bestätigte Scope-Zeile ändert. Gefunden beim Nachziehen von ADR-013 |
