...
| **S-49** `join_code` erhält zusätzlich zur bestehenden Rotation (G-A5 — ergänzt sie, ersetzt sie nicht) einen Ablauf (`join_code_expires_at`, Vorschlag 7 Tage) und eine Nutzungsgrenze (`join_code_max_uses`, vorbelegt mit der Zahl fehlender Bewohnender), beide in `HouseholdSettings` änderbar, sowie einen Warnhinweis auf der Teilen-Seite („nur direkt mit Mitbewohnenden teilen — wer ihn hat, kann mitstimmen"). Seit **S-03** die E-Mail-Pflicht beim Beitritt gestrichen hat und mit **S-05** die für Bewohnende sichtbare Bewohnerliste samt Entfernen-Recht entfällt, ist der Link die **einzige verbliebene Zugangskontrolle** — diese Absicherung ist deshalb Voraussetzung, nicht Verbesserung. **Nachtrag 2026-09-21, drei Präzisierungen am Wortlaut dieser Zeile, keine Scope-Änderung:** (1) die Vorbelegung der Nutzungsgrenze ist seit **O-15** (2026-09-16) **1** (Einmal-Code) und nicht mehr „die Zahl fehlender Bewohnender" — der alte Vorschlag wurde ersetzt, nicht ergänzt; (2) Ablauf und Grenze waren nie in `HouseholdSettings` änderbar, sondern lagen am `Household` — und liegen seit **O-18** (aufgelöst 2026-09-21) an **je einem ausgestellten Link** (`JoinCodeIssuance`, `domain/identity.md` §2.1); (3) die Rotation aus G-A5 heißt seither **Löschen** und wirkt je Link. Was S-49 verlangt — Ablauf, Nutzungsgrenze, Warnhinweis — ist unverändert. Die Zahl der Links ist eine Anforderung, keine Scope-Zeile: `F2-requirements.md` **FR-2.1** | Verifikation, zweiter Faktor, Geräte-Fingerprinting (§ 25 TDDDG) |
| **S-50** Ein Konto ohne aktives `ResidentProfile` (Verwaltung) erreicht ausschließlich Haushaltsverwaltung — Zimmer, Mitglieder, Beitrittscode, Verfahrensregeln, Aufbewahrung; `Application`, `Slot`, `Appointment` und `CastingNote` setzen ein `ResidentProfile` voraus. **Präzisiert durch ADR-014:** `CastingRound` ist davon ausgenommen, aber nur in Identität und Lebenszyklus — Existenz, `title`, `status`, `room_ids`, Zeitstempel und Aufbewahrungsfelder. **Alles aus `Application` Abgeleitete bleibt unsichtbar, ausdrücklich einschließlich Zahlen** (Bewerbungszahl, Beteiligung, Score, Rangliste). Ohne diese Ausnahme sind O17, S-35 und `Room → not_available` nicht umsetzbar; ohne die Grenze wäre V-1 über den geteilten Haushaltszugang umgangen (geschützter Test **G-D15**). Zwei benannte Ausnahmen bleiben bei der Verwaltung: Aufbewahrung (verlängern/kürzen/löschen/archivieren) und Datenauskunft **als Export ohne Einsicht**. Wird der letzte Moderator handlungsunfähig, kann sich die Verwaltung jederzeit selbst ein `ResidentProfile` anlegen und Moderatoren ernennen — ein benannter Handelnder statt direktem Zugriff. **Keine Sicherheitsgrenze:** wer die Haushaltszugangsdaten kennt, kann sich weiterhin ein Profil anlegen und handeln; was sich ändert, ist Zurechenbarkeit, nicht Zugriffsschutz | Verwaltung mit direktem Lese- oder Schreibzugriff auf Beratungsinhalte; ein zweiter, undokumentierter Zugriffspfad |
...
|-------|--------|------|--------------|
| **v0.1 — Vertikaler Schnitt** | **Fundament, nicht nachrüstbar:** S-31 (Sichtbarkeitsinvariante), S-36 (Autorisierung doppelt), S-15 (Zustandsmaschine vollständig, Rückwärtsübergänge auditiert), S-37 (`data-inventory.yml` als CI-Gate) · **Identität und Zugang:** S-01, S-02, S-03, S-04, S-05, S-49, S-50 · **Runde und Zimmer:** S-06, S-07 · **Erfassung, nur Formularweg:** S-08 (Formularhälfte), S-38 · **Screening und Stimme:** S-09, S-10 · **Rangliste:** S-12, S-13, S-14 · **Aufgaben-Dashboard:** S-48 · **Hilfsmittel und Protokoll:** S-16, S-27 (Append-only-Log; die Feed-Oberfläche liegt später), S-35 · **von S-33 nur die Handlöschung** (manuelles Löschen je `Application`; die Automatik samt Vorwarnung liegt im Gate in v0.2) | **„Ein Haushalt screent und rankt echte Bewerbungen von der Erfassung bis zur Rangliste, ohne nach WhatsApp auszuweichen."** Die Beteiligungsquote wird hier zum ersten Mal messbar. Läuft auf **synthetischen Daten**; der Weg zu echten Bewerberdaten führt über das Gate in v0.2 | — |
| **v0.2 — Vollständige Runde** = **v1** | S-08 (Paste-Parser-Hälfte) samt **S-39** (Absätze verwerfen) · S-11 und S-47 (zweiter Durchlauf) · **S-44** (weiche Rundenfrist samt Restzeit-Anzeige) · S-17 (manuelle Erfassung; die bewerberseitige Token-Seite liegt in v1.1) · S-18 · S-21, S-51 · S-22, S-46 · S-23, S-24 · S-25, S-42 · S-28, S-29 · S-30, S-45 · S-32 · S-40 · S-41 (**Datenmodell**) · **Gate vor dem ersten echten Haushalt:** S-33 (**Aufbewahrungsautomatik** samt 14-Tage-Vorwarnung — die manuelle Einzellöschung liegt bereits in v0.1), S-34 (Datenauskunft-Export), Klick-AVV, Art.-13-Datenschutzseite je Haushalt im Zustand `draft → published` (G-C9), Liste der Unterauftragsverarbeiter | **Unverändert das v1-Ziel:** „Ein echter Haushalt führt eine vollständige Runde von der ersten `Application` bis `moved_in` durch, ohne nach WhatsApp auszuweichen." Mit dem Abschluss von v0.2 ist **v1 erreicht** | v0.1 |
| **v1.1** | **S-19, S-20** (Solver „Vorschlag berechnen" samt Erklärbarkeit) · **S-26** (Kalenderansicht) · **Bewerberseitige Token-Seite** für Verfügbarkeiten (Datenmodell liegt in v0.2, S-17, O-08) · **Oberfläche für `subject_statement`** (S-41) · Punkte-Budget-Variante als wählbare Option · Textbausteine für die Kontaktaufnahme · **S-43** Spenden-E-Mail nach der 3.–4. abgeschlossenen `CastingRound`, außerhalb des In-App-Flows (löst O-05; bedingt durch H-V4, erst nach der Concierge-Runde geprüft) | „Der Komfort kommt nach, ohne dass der vollständige manuelle Pfad davon abhängig war." Jedes v1.1-Feature ist bewusst eine **Bequemlichkeit über einem vollständigen manuellen Pfad** (P-1) | v1 |
...
**Warum S-44 in v0.2 sitzt und nicht in v0.1.** Eine weiche Rundenfrist ist optional und
blockiert nichts (analog Quorum, S-13). Der Schnitt v0.1 kommt ohne sie aus: Solange nur **eine**
Aufgabenart offen ist — die Stimme —, hat die Vorrangregel S-48 nichts zu sortieren und fällt auf
ihre feste Reihenfolge zurück. Erst wenn Verfügbarkeiten, Slot-Reaktionen und Notizen gleichzeitig
offen sein können, liefert S-44 das Datum, das S-48 braucht. Beide Karten der Frist auf dem Board
(„set a soft deadline", „see how much time is left") liegen deshalb in v0.2.

**Zwei S-Zeilen sind zwischen den Stufen geteilt, zusätzlich zu S-08/S-17/S-41.** **S-33** —
die Handlöschung je `Application` liegt in v0.1, weil Testdaten sonst nicht wegzuräumen sind; die
Automatik mit Vorwarnung ist Teil des Gates. **S-39** gehört zum Parser, nicht zum Formular: die Zeile sagt selbst „im
Paste-Parser-Schritt", und in einem reinen Formularweg gibt es keine Rohnachricht, aus der
Absätze zu verwerfen wären — wer den Text tippt, tippt sie einfach nicht. Die Zeile ist deshalb
mit der Parser-Hälfte von S-08 nach v0.2 gewandert. **S-10** — die Revidierbarkeit der Stimme ist
ausdrücklich Teil der Zeile („revidierbar innerhalb derselben `Vote.stage`, solange die
`CastingRound` `open` ist") und liegt damit vollständig in v0.1, nicht erst in v0.2.

**Warum S-19/S-20 und S-26 aus v1 nach v1.1 wandern.** Beide sind Bequemlichkeiten über einem
Pfad, der ohnehin vollständig existieren muss: der Solver über dem **manuellen Legen von Slots
und der Feasibility-Schicht ohne Solver** (S-18, in v0.2), die Kalenderansicht über den bereits
gespeicherten `Appointment`s und Einzugsterminen. Damit gilt für sie genau die Begründung, die
dieses Dokument unten schon für den Verfügbarkeits-Link führt. Der Solver ist zugleich die
**teuerste Einzelentscheidung der Architektur** (ADR-005) — ihn vor dem ersten vollständigen
Rundendurchlauf zu bauen, ist die Reihenfolge, die die Aufwandsgegenprobe gerade ausschließt.

**Warum die Sichtbarkeitsinvariante (S-31) und die Autorisierungsschicht (S-36)
vollständig in v0.1 liegen:** Beide bestimmen jede Abfrage im System. Nachträglich eingezogen
...
Zustände zu migrieren. Und für **S-37**: ein CI-Gate, das erst später eingezogen wird, trifft
auf ein Schema, das längst nicht deklarierte Spalten enthält — `04-Domaenenmodell.md` §9 ist
genau daran dreimal verrutscht.
...
| **Keine Gamification des Sozialen** | Beteiligungsanzeigen benennen den Stand der Gruppe, nicht das Versäumnis Einzelner. Kein Ranking unter Bewohnenden, kein öffentlicher „hat noch nicht abgestimmt"-Pranger. **Begründete Ausnahme (2026-09-16, Prototype-User-Test):** Die Rangliste (D1) darf, nachdem die betrachtende Person selbst abgestimmt hat, anzeigen, welche Stimmberechtigten bereits abgestimmt haben — Teilnahme, nicht Inhalt. Zu diesem Zeitpunkt sind die Ergebnisse für diese Person ohnehin schon aufgedeckt (V-4); die Teilnahme-Anzeige eröffnet daher kein zusätzliches Anker-/Bandwagon-Risiko und unterstützt stattdessen das Gespräch innerhalb der WG. Die Teilnehmendenliste (B4) bleibt von dieser Ausnahme unberührt und zeigt weiterhin keinen Abstimmungsstatus |
...
| ~~**O-02**~~ **entschieden (2026-09-08)** | Aufbewahrungsfrist für `ActivityEvent` und Audit-Einträge — **Beschluss: das Ereignis bleibt, die Nutzlast geht.** Ein `ActivityEvent` wird nie gelöscht (append-only, ADR-003); seine personenbezogene Nutzlast wird in **derselben Transaktion** redigiert wie die Bezugsdaten, auf die sie zeigt, und der Eintrag auf Typ, Zeitpunkt, handelndes Profil und „Inhalt gelöscht" reduziert. Damit trägt die 180-Tage-Frist aus S-33 auch für das Log, ohne die Rechenschaftsfunktion nach Art. 5 Abs. 2 aufzugeben, und das Log wird nicht zum Umweg um die Sichtbarkeitsinvariante (G-D7/G-D8). Nötig für **v0.1**, weil dort ab dem ersten Zustandsübergang auditiert wird. Ursprüngliche Abwägung: | Der Feed enthält personenbezogene Ereignisse; die 180-Tage-Frist (S-33) ist für `Application`, `Vote`, `CastingNote` festgelegt, nicht für das Log. Ein Audit-Log, das mit den Daten gelöscht wird, kann seine Rechenschaftsfunktion nicht erfüllen — eines, das bleibt, hält Beratungsinhalte fest | `06-Compliance-Anhang.md` und `04-Domaenenmodell.md`. Tendenz: Ereignisse behalten, personenbezogene Nutzlast beim Löschen der Bezugsdaten mitlöschen und den Eintrag auf „gelöscht" reduzieren |
...
| **O-06** | Grenzwerte des Solvers (Anzahl Bewerbende × Slots × Bewohnende), ab denen die Rechenzeit unzumutbar wird | Bestimmt, ab welcher Problemgröße das Zeitlimit regelmäßig greift | **Das Verhalten am Limit ist geklärt** (S-20c: beste zulässige Lösung, gekennzeichnet; sonst Unlösbarkeits-Erklärung mit fester Relaxationsreihenfolge). Offen bleibt allein die Größenordnung → `05-ADRs.md` (ADR-005), Zeitbudget in `03-PRD.md` §6.1. **Neu einsortiert:** Der Solver liegt mit S-19/S-20 in v1.1, die Messung damit ebenfalls. Geführt als Implementierungspflicht im Register, nicht mehr als Planungsschuld. |
| **O-07** | Baseline-Erhebung im Testhaushalt (§8.3) | Nach dem ersten Einsatz nicht mehr rekonstruierbar | Kein Dokumentations-, sondern ein Terminproblem — und in **zwei Hälften** zerlegt, weil nur eine davon noch von einem Termin abhängt.<br>**(1) Rückblickend, sofort erhebbar:** Nachrichtenzahl, Tage von erster Bewerbung bis Entscheidung und Zahl der Antwortenden lassen sich **jetzt** aus den Chatverläufen der letzten Runde rekonstruieren. Das wird mit jedem Monat schwerer, aber es ist heute noch möglich — diese Hälfte hängt an keinem Termin mehr.<br>**(2) Vorausschauend:** Organisationsaufwand und empfundene Beteiligung brauchen eine laufende Runde. Die geplante Concierge-Runde entfällt voraussichtlich, weil die Prototypen früher stehen; diese Hälfte wandert damit zur Erhebung an den Prototypen. |
| ~~O-08~~ | ~~Wird die **bewerberseitige Token-Seite** für Verfügbarkeiten aus v1.1 nach v1 vorgezogen?~~ | — | **Geklärt (2026-09-09):** Nein. v0.1 ist der vertikale Schnitt Bewerbung → Screening → Stimme → Ergebnis; Verfügbarkeiten kommen darin nicht vor, die Seite kann also nicht vorgezogen werden. Bleibt v1.1. **Tragende ID für diese Frage** — `03-PRD.md` P-O-07 und `04-Domaenenmodell.md` O-11 verweisen hierher. Maßgeblich: `02-SRD.md` §5.4 |
| ~~O-09~~ | ~~**S-50 sagt, `CastingRound` setze ein `ResidentProfile` voraus — drei bereits beschlossene Verwaltungsrechte widersprechen dem.** (1) Bildschirm **O17 Aufbewahrung** verwaltet „Aufbewahrungsfristen abgeschlossener Runden" und ist ausdrücklich ohne `ResidentProfile` erreichbar. (2) **S-35** verbietet Änderungen am Abstimmungsverfahren „nicht während laufender Runde" — das setzt voraus, dass die Verwaltung eine laufende Runde erkennen kann. (3) `Room` → `not_available` heißt „fällt aus der **laufenden Runde**". Auch `invarianten.md` §5.2 lässt den Haushalts-Account Runden sehen~~ | — | **Geklärt (2026-09-14) durch ADR-014: S-50 präzisiert, V-2 unverändert.** Sichtbar ist **Rundenidentität und Lebenszyklus** — Existenz, `title`, `status`, `room_ids`, Zeitstempel, Aufbewahrungsfelder. Unsichtbar bleibt **alles aus `Application` Abgeleitete, ausdrücklich einschließlich Zahlen**: `redaction_subjects()` ist für einen Haushalts-Account **leer** (ADR-013), V-1 greift dort nicht, und Aggregate sind nach `invarianten.md` §5.1 V-1-geschützt. Erzwungen durch **G-D15**. `CastingRound.title` bleibt sichtbar — bewusst akzeptiert, weil O17 sonst unbedienbar wird; die Anleitung zu sachlicher Benennung gehört in den Textkatalog aus `03-PRD.md` P-O-04. Maßgeblich: `adr/0014-*.md` |
