...
| `created_by_profile_id` | `uuid` | 🟠 | **`NOT NULL` seit O-17** — dieselbe Begründung wie bei `Application.created_by_profile_id` (§2.2): Terminfindung setzt ein `ResidentProfile` voraus, seit S-50/U-20 kann niemand mehr `null` erzeugen |
