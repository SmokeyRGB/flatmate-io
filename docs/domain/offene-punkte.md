...
| ✅ O-15 | Standardwerte für Ablauf/Nutzungsgrenze des Beitrittscodes (vormals Plan-O-B). *(Die Felder hießen bis 2026-09-21 `join_code_expires_at`/`join_code_max_uses` am `Household`; seit **O-18** liegen sie als `expires_at`/`max_uses` an `JoinCodeIssuance`. Die Standardwerte selbst sind unverändert.)* | **Ablauf 7 Tage** — Vorschlagswert, jederzeit über `Household.join_code_expires_at` änderbar, kein hartcodierter Wert. **Nutzungsgrenze, aktualisiert 2026-09-16:** Standard **1** (Einmal-Code) aus Sicherheitsgründen — ein Link soll standardmäßig nur eine Person einlösen können. **Ausnahme: der Gründungs-Link** (unmittelbar nach `A1 Registrierung`), der bewusst für mehrere Personen vorbelegt wird — Vorschlagswert **Zahl der bei der Gründung angegebenen erwarteten Bewohnenden** — weil er einmalig geteilt wird (z. B. im Gruppenchat), damit alle Gründungsmitglieder darüber beitreten. Spätere Einladungen (auch die reguläre Einladung nach einer Zusage) bleiben Einmal-Codes. Ursprünglicher Vorschlag „Zahl der noch fehlenden Bewohnenden" als allgemeiner Standard ist damit **ersetzt**, nicht nur ergänzt — jederzeit über `Household.join_code_max_uses` änderbar | §2.1 |
...
| ~~O-18~~ *(neu 2026-09-16, Prototype-User-Test; aufgelöst 2026-09-21)* | ~~**`join_code`-Historie fehlt.** Das Modell trägt nur einen einzelnen, rotierenden Code mit Nutzungszähler, keine Historie einzelner Ausstellungen~~ | §2.1 | **Geklärt 2026-09-21 — zugunsten der Historie.** Neue Entität **`JoinCodeIssuance`**: ein Haushalt stellt mehrere Links aus, jeder mit eigener Frist, eigener Nutzungsgrenze, eigenem Zähler und eigenem `deleted_at`; `Membership.joined_via_issuance_id` (vormals `joined_via_code`) verknüpft die Mitgliedschaft mit dem Link, über den sie entstand. Die fünf `join_code*`-Spalten verlassen `Household`. **Das gefrorene `04-Domaenenmodell.md` wird nicht angefasst** — es bleibt der V0.4-Schnappschuss; die lebende Fassung ist `domain/identity.md` §2.1. Maßgeblich: `review-log.md` §Offene-Punkte-Register |
...
| O-19 *(neu 2026-09-24)* | **Kein Löschdatum für eine `Application` ohne abgeschlossene Runde.** Der Aufbewahrungs-Anker ist der Abschluss der `CastingRound` (`06-Compliance-Anhang.md` §5.3). Eine `Application`, deren Runde nie geschlossen wird oder die zu keiner Runde gehört (`round_id` ist nullbar), bekommt damit nie ein `retention_until` — und fällt aus der Löschautomatik | §2.2, `aufbewahrung.md` §7 | Muss entschieden sein, bevor der Aufbewahrungslauf gebaut wird: sonst bleiben genau diese Bewerbungen unbegrenzt gespeichert (Art. 5 Abs. 1 lit. e). Rechtlicher Teil bei `06-Compliance-Anhang.md` §5 |
...
| ~~O-17~~ | ~~**`created_by_profile_id` ist nullbar, aber seit S-50/U-20 kann niemand mehr `null` erzeugen.** Betrifft `Application` (§2.2) und `AvailabilityToken` (§2.4)~~ | §2.2, §2.4 | **Geklärt (2026-09-14): beide Spalten werden `NOT NULL`.** Der Urheber ist immer ein `ResidentProfile`; die Nullbarkeit war eine Altlast aus der Zeit vor S-50/U-20, als die Verwaltung Bewerbungen anlegen und Termine planen durfte. Verworfen wurde, `null` als „vom System erzeugt" umzudeuten: Es gibt heute keinen Systempfad, und käme einer, bekäme er eine **benannte** Quelle — „kein Wert" und „vom System" dürfen nicht gleich aussehen. Maßgeblich: `casting.md` §2.2, `scheduling.md` §2.4 |
