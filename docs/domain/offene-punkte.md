...
| **O-17** | **`created_by_profile_id` ist nullbar, aber seit S-50/U-20 kann niemand mehr `null` erzeugen.** Betrifft `Application` (§2.2) und `AvailabilityToken` (§2.4). Beide Felder stammen aus der Zeit, als die Verwaltung Bewerbungen anlegen und Termine planen durfte; seither setzen beide Vorgänge ein `ResidentProfile` voraus | §2.2, §2.4 | Zwei Auflösungen sind denkbar und schließen einander aus: **(a)** die Spalten werden `NOT NULL` — dann ist der Urheber immer ein Profil, und die Altlast verschwindet; **(b)** `null` bekommt eine **neue** Bedeutung „vom System erzeugt" (etwa für Importe oder den automatischen Token aus S-42) — dann braucht es eine benannte Quelle statt eines stillen `null`. **Kein Beschluss ohne (b) geprüft zu haben:** Solange kein Systempfad benannt ist, ist (a) die ehrlichere Wahl. Gefunden beim Nachziehen von ADR-013 |
