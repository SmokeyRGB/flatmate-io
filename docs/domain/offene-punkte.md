...
| O-18 *(neu, 2026-09-16, Prototype-User-Test)* | **`join_code`-Historie fehlt.** Das Modell trägt nur einen einzelnen, rotierenden Code mit Nutzungszähler (`join_code`, `join_code_uses`), keine Historie einzelner Ausstellungen. Der Lovable-Prototyp zeigt auf O16 mehrere Links (aktiv + aufgebraucht) nebeneinander und will pro aufgebrauchtem Link zeigen, wer sich darüber registriert hat — das verlangt eine schlanke `JoinCodeIssuance`-artige Historie statt eines einzelnen rotierenden Werts | §2.1 | Betrifft `04-Domaenenmodell.md` (frozen) — keine Entscheidung hier vorweggenommen, weil sie das gefrorene Snapshot verändert. Vorschlag: je Ausstellung Code, Gültigkeit, Nutzungslimit, Verbrauchsstatus, verknüpft über `Account.joined_via_code` mit dem beigetretenen Profil. |
