...
| **Produktivdatenbank nachziehen** *(neu 2026-09-25)* | Seit der Trennung der Testumgebung (2026-09-18) sind alle Migrationen nur auf `flatmate-io-dev` angewendet. `flatmate-io` (Produktion) steht auf `drizzle/0012`; **ab `0013` fehlt alles**. Stand 2026-09-25 per Abfrage beider Projekte geprüft. Beim Nachziehen: `0013`–`0015` enthalten `SECURITY DEFINER` und laufen von Hand im SQL-Editor; eine Enum-Erweiterung (`0016`) läuft allein und vor ihrer ersten Verwendung. Produktion enthält noch die Testdaten aus der Zeit vor der Trennung, und `activity_event` ist unlöschbar — zu entscheiden ist daher, ob migriert oder neu aufgesetzt wird | **Menschliche Entscheidung 2026-09-25:** bewusst aufgeschoben bis zum Abschluss von v0.1, nicht pro Änderung. Vor dem ersten echten Haushalt fällig |
