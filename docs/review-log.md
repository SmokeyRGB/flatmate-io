...
| **`02-SRD.md` O-09** *(neu 2026-09-14)* | **S-50 verbietet dem Haushalts-Account `CastingRound` — drei beschlossene Verwaltungsrechte brauchen es trotzdem:** Bildschirm O17 (Aufbewahrungsfristen abgeschlossener Runden, ausdrücklich ohne `ResidentProfile` erreichbar), **S-35** (Verfahrensänderung „nicht während laufender Runde") und `Room → not_available` („fällt aus der laufenden Runde"). `invarianten.md` §5.2 lässt die Sicht bereits zu | **Braucht einen eigenen Record** — es ändert eine bestätigte Scope-Zeile. Vorschlag steht in `02-SRD.md` §11: Rundenidentität und Lebenszyklus sichtbar, alles aus `Application` Abgeleitete **einschließlich Zählern** unsichtbar. Die Zähler sind der Punkt: `redaction_subjects()` ist für einen Haushalts-Account leer (ADR-013), V-1 greift dort nicht, und Aggregate sind V-1-geschützt |
| **`03-PRD.md` P-O-10** *(neu 2026-09-14)* | **Zwei Dokumente nennen verschiedene Tore für dieselben Zimmer-Übergänge:** Rechtematrix `Room` ✅ für den Moderator, `zustandsmaschinen.md` §3.3 `manage_settings` — das die Matrix dem Moderator nur ⬜ gibt | Nicht kosmetisch: Gilt §3.3, braucht ein Moderator ohne `manage_settings` für jedes `on_hold` die Verwaltung, mitten in der Runde — und das Favoriten-Budget zählt nur `open`. Tendenz: eigenes Recht `manage_rooms`, oder §3.3 nennt die Rolle statt der Berechtigung |
| **`04-Domaenenmodell.md` O-17** *(neu 2026-09-14)* | **`created_by_profile_id` ist nullbar, aber seit S-50/U-20 erzeugt niemand mehr `null`** — betrifft `Application` und `AvailabilityToken` | Entweder `NOT NULL`, oder `null` bekommt die neue Bedeutung „vom System erzeugt" samt benannter Quelle. Solange kein Systempfad benannt ist, ist `NOT NULL` die ehrlichere Wahl. Schemafrage, daher `04`/`domain` zuständig |
| **`07-Screen-Inventar.md` O-G** *(neu 2026-09-09)* | Die **Übersetzungstabelle für UI-Vokabular** (U-24: kein Modellbegriff erscheint auf einem Bildschirm ungeklärt) ist an zwei Stellen als „§8.6" zitiert — im Kopf des Dokuments und in §12 als Barrierefreiheits-Anforderung — **existiert aber nicht.** `07-Screen-Inventar.md` führt keinen Abschnitt §8.x; `04-Domaenenmodell.md` §8.6 ist `phase_hint` und damit etwas anderes | **Beim Umbau gefunden, nicht erfunden.** Die Tabelle ist Spezifikationsarbeit: sie braucht je Modellbegriff ein deutsches UI-Wort. Blockiert v0.1 nicht — betrifft sichtbaren Text, und U-24 nennt mit „Quorum → genug Stimmen" bereits das Muster. Fällig, bevor Oberflächentexte final werden. Neuer Ort: `screens/rahmenwerk.md` |
