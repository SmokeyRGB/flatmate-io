...
| **`04-Domaenenmodell.md` O-18** *(neu 2026-09-16, Prototype-User-Test)* | `join_code`-Historie fehlt: das Modell trägt nur einen einzelnen, rotierenden Code mit Zähler, keine Historie einzelner Ausstellungen mit Nutzer-Zuordnung | Betrifft `04-Domaenenmodell.md` (frozen) — Entscheidung, ob eine `JoinCodeIssuance`-artige Historie ergänzt wird, steht aus. Siehe `domain/offene-punkte.md` O-18, `screens/O-organisation.md` O16 |
...
| **ADR-006 (2026-09-16)** | `SET LOCAL`-Sicherheit unter Supabases Transaktions-Pooler ist unverifiziert | Der geschützte Test **G-D10/AC-0.7** existiert bereits als Anforderung (`GUARDRAILS.md`, Minimal-Gate Punkt 7), muss aber **vor der ersten Policy** (`MINIMAL-GATE.md` Punkt 7) unter dem Pooler laufen, nicht nur unter einer Direktverbindung — er ist jetzt die einzige, nicht mehr eine von zwei Verteidigungslinien |
