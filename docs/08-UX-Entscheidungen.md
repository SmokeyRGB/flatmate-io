...
## 1. Die Entscheidungen U-1 bis U-30
...
| **U-12** | **Einladungslink:** Warnhinweis beim Teilen, Ablauf, Nutzungsgrenze *(K-2)* — **weiterhin offen**, und dringlicher denn je | **geschlossen** als S-49; Vorbelegung O-15, **Nutzungsgrenze-Standard seit 2026-09-16 auf 1 (Einmal-Code) mit Gründungs-Link-Ausnahme aktualisiert**. **Nachtrag 2026-09-21 (O-18 aufgelöst):** Ablauf und Nutzungsgrenze hängen seither an **je einem ausgestellten Link** (`JoinCodeIssuance`), nicht am Haushalt — ein Haushalt kann mehrere Links gleichzeitig offen haben, jeder einzeln löschbar, alle in einer Historie sichtbar. Der Warnhinweis bleibt unverändert. Maßgeblich: `domain/identity.md` §2.1, `F2-requirements.md` FR-2.1 |
...
| **U-20** | **Verwaltung erreicht keine Bewerbungsdaten.** Ohne `ResidentProfile` nur Haushaltsverwaltung plus `CastingRound`-Identität/Lebenszyklus (Existenz, `title`, `status`, `room_ids`, Zeitstempel, Aufbewahrungsfelder) — alles aus `Application` Abgeleitete bleibt unsichtbar, ausdrücklich einschließlich Zahlen; Ausnahmen: Aufbewahrung und Datenauskunft-Export | S-50, präzisiert durch ADR-014; zweite Kommentarrunde |
...
| **U-22** | **Zwei getrennte Listen:** Teilnehmendenliste (alle Bewohnenden, nur Namen) und Bewohnerliste (Verwaltung voll, Moderator lesend, Residents gar nicht) — **die Moderator- und die Residents-Zeile sind durch U-30 (2026-09-17) präzisiert, nicht mehr aktuell wie hier ursprünglich gefasst** | S-05; kippt U-16 und zwei der vier Duplikatsschutz-Mechanismen |
...
| **U-30** | **Moderatoren erhalten volle Bewohnerlisten-Rechte (Parität mit Verwaltung)**, statt nur lesend (präzisiert U-22). **Bewohnende erhalten eine eigene, reduzierte „Wer wohnt hier"-Ansicht** — nur `status = active`, keine Aktionen, keine Kontaktdaten, kein Beitrittsdatum, keine eigene Route in die Bewohnerliste oder Teilnehmendenliste — damit eine bewohnende Person eine über den Beitrittscode fälschlich oder böswillig beigetretene Person (Eindringling) erkennen und außerhalb der App melden kann. Stellt einen Teil des durch U-22 verlorenen Sichtbarkeits-Schutzmechanismus (E-06) wieder her, ohne das Entfernungsrecht zurückzugeben | `F1-requirements.md` FR-1.31/AC-1.24, C-1.10; screens `B-start.md` B5, `O-organisation.md` O16; entschieden 2026-09-17 |
...
Keine. Alle 30 Entscheidungen sind entschieden — das war der Zweck des Abschnitts „Entschieden"
im Plan (U-1…U-26), des Prototyp-Abgleichs vom 2026-09-16 (U-27 bis U-29) bzw. der F1-Umsetzung
vom 2026-09-17 (U-30). Zwei tragen einen Nachtrag:
