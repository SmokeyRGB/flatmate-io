...
- Lists & status badges: a plain list row (e.g. "who's voted so far") pairs a name on the left with a status pill on the right — outlined pill for a pending/neutral state ("stimmt mit ab"), a muted filled pill for an inactive-but-historical state ("ausgezogen" / moved out), and the same outlined-but-inert treatment for an entity that structurally cannot act ("stimmt nicht ab" for a shared household account). A richer roster row (household members under Organisation) adds an optional solid role badge chip (shield icon + label, e.g. "Moderation") next to the name, and a top-right icon-only brick-red trash button in the card header for the one irreversible leave action (Entfernen = irreversible removal, gated by the type-to-confirm dialog), kept out of the row of actions below so its wrap behavior never depends on the label width of the toggle button next to it. That action row holds neutral secondary buttons for both reversible administrative toggles (Kontakt, Moderation geben/entziehen) and the other, also-reversible way a person leaves (Ausgezogen = reversible move-out) — same secondary pill shape/height throughout, so red is reserved solely for the one irreversible action (Entfernen), and the confirmation step (typed name) is what signals danger there, not color.
