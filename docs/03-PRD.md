...
| 5 | Bewerbung erfassen | Formular ausfüllen **oder** Nachricht einfügen und Parser-Vorschlag bestätigen | `Application` im Zustand `new` (P-1, S-08) | 2, 3 | Parser erkennt nichts → Formular mit dem **Rohtext im Freitextfeld** und ohne geratene Felder, keine Fehlermeldung (§4.1.3); Pflichtfeld ist nur `name` |
...
| Eher nicht | 1 | |
| Finde gut | 3 | |
| **Unbedingt** | 5 | „= dein Favorit" — es gibt keinen **eigenen Favoriten-Durchlauf nach dem Screening** (E-07). Der kurze zweite Durchlauf bei Budgetüberschreitung (§4.1.5, S-11/S-47) bleibt davon unberührt: er bewertet dieselben Karten erneut, er wählt keine Favoriten aus |
...
| **Abstimmung** | Zweiter Durchlauf an/aus (ersetzt „Feinschliff an/aus", S-47) · **Favoriten-Budget-Faktor `favorite_budget_factor`** (Standard `1,5`; die Schwelle ist `max(1, ceil(offene Zimmer × Faktor))` und wird nicht direkt gesetzt — löst den Widerspruch, dass §4.2.2 eine „Feinschliff-Schwelle" sperrt, die hier bisher kein Feld hatte) · verdeckte Ergebnisse an/aus (Standard an) · Quorum-Schwelle `quorum_share` (Standard `0,5`) · Veto: Begründungspflicht (Standard an), Budget (Standard 1), Anonymität (Standard aus) — **alle bei offener Runde gesperrt** (§4.2.2) |
...
| ~~P-O-10~~ | ~~**Welches Recht trägt die Zimmer-Verfügbarkeit?** Rechtematrix `Room` ✅ für die Moderation, `zustandsmaschinen.md` §3.3 dagegen `manage_settings`, das dieselbe Matrix der Moderation nur ⬜ gibt~~ | — | **Geklärt (2026-09-14): neues Recht `manage_rooms`**, vorbelegt bei `household_admin` und `moderator`. Die Verfügbarkeit eines Zimmers ist eine Entscheidung der laufenden Runde und wirkt direkt auf das Favoriten-Budget (nur `open` zählt); `manage_settings` bündelt dagegen Verfahrensregeln und Datenschutzseite und wäre das falsche Tor. `promised`/`occupied` bleiben bei `change_application_state`, `occupied → open` bei `manage_members`. Maßgeblich: `domain/identity.md` §2.1 und `domain/zustandsmaschinen.md` §3.3 |
