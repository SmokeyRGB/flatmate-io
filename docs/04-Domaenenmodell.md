...
| `reveal_vote_authorship` | `bool` | ⚙️ | **Neu (2026-09-16, Prototype-User-Test, nachgezogen aus `domain/identity.md`).** Default `false`. Wenn aktiv, zeigt die Kandidaten-Einzelansicht (D2) zusätzlich zur aggregierten Verteilung, welche Person wie gestimmt hat. Selbst-Redaktion (G-D1) und der Auskunftsexport-Anonymitätsschutz (G-D6) gelten unverändert und unabhängig von dieser Einstellung |
