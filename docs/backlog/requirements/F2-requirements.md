...
| **EC-2.6** *(corrected 2026-09-24)* | A resident loses their password and never added an email | The administration (household account only) can issue a single-use password-reset link for that active profile; redeeming it sets a new password, ends every session of the profile, and is recorded as `account.password_reset_by_admin`. It is available only while the profile's account has no email — adding one closes the gap. Stated plainly rather than hidden — the reason the email pitch exists at all. Source: `domain/identity.md` §2.1 (O-16); `03-PRD.md` §4.1.1 (K-18). The previous wording ("recovery is impossible; a moderator must create a fresh profile") contradicted both and is corrected. Decision recorded in `review-log.md` §Offene-Punkte-Register |
